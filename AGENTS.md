...
- CLI build/test (macOS):
  ```bash
  xcodebuild -project aibotwithfeelings.xcodeproj -scheme aibotwithfeelings -destination 'platform=macOS' test
  ```
...
| `.github/workflows/swift.yml` | Linux core tests + macOS `xcodebuild test` |
...
2. **macos-xcode-tests** — `xcodebuild test` with the shared `aibotwithfeelings` scheme
